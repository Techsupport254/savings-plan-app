...
const Savings = () => {
  ...
  const handleSaveToday = () => {
    ...
    setNotifications([...notifications, { message: `Successfully saved KES ${today}`, date: new Date() }]);
  };

  const handleSavePrevious = () => {
    const previousDates = getPreviousDates(savedDates);
    setCurrentSavings(currentSavings + previousDates);
    setSavedDates([...savedDates, ...Array.from({ length: previousDates }, (_, i) => i + 1)]);
    setNotifications([...notifications, { message: `Successfully saved KES ${previousDates}`, date: new Date() }]);
  };

  return (
    <div className="savings">
      <div className="top-grid">
        <CardSavings currentSavings={currentSavings} handleSaveToday={handleSaveToday} handleSavePrevious={handleSavePrevious} />
      </div>
      <div className="middle-grid">
        <Table savedDates={savedDates} />
      </div>
      <div className="bottom-grid">
        <Progress savedDates={savedDates} />
      </div>
    </div>
  );
};
